 ...
 */
function Request() {
  this.get = function(query, func) {    
    // Header of the request : the session ID need to be transmitted
    var headers = {"Cookie": query.cookie};
    ...
    var response = UrlFetchApp.fetch(query.url, options);
    
    //writelog("Response: "+response.getContentText());
    
    if(func)
      func(false, response, response.getContentText());
  };
  
  this.post = function(query, func) {    
    // Header of the request : the session ID need to be transmitted
    var headers = {};
    
    if(query.cookie)
      headers = {"Cookie": query.cookie};
...
function GetDatas(QUERY,ONSUCCESS_FN, ONERROR_FN,ON_AFTERGETDATAS) {
  ...
  request.get(QUERY, function ( err, res, body){  
    if (err || !(res.getResponseCode() >= 200 && res.getResponseCode() < 400)) {
...
function CommonRunScene_FN(req,ONSUCCESS_FN,ONERROR_FN,ON_AFTERRUNSCENE){
  GetDatas(req,
           function( err, res, body){
             ...
             if (ONSUCCESS_FN) ONSUCCESS_FN("ok ");
             if(ON_AFTERRUNSCENE) ON_AFTERRUNSCENE(result);		
           },
           function( err, res, body){
             if (ONERROR_FN) ONERROR_FN("error " + ((err) ? err : ""));
	...
	if (device){
		if (device.attributes){
			if (device.attributes[attribute_id]){
				...
				if (ONERROR_FN) ONERROR_FN("device has no attribute " + attribute_id);	
			...
			if (ONERROR_FN) ONERROR_FN("device has no attributes!");
			if(zipabox.events.OnAfterSetDeviceValue) zipabox.events.OnAfterSetDeviceValue(null);
			if(ON_AFTERSETDEVICEVALUE) ON_AFTERSETDEVICEVALUE(null);
		}
		...
		if (ONERROR_FN) ONERROR_FN("unknow uuid " + uuid);
		if(zipabox.events.OnAfterSetDeviceValue) zipabox.events.OnAfterSetDeviceValue(null);
		if(ON_AFTERSETDEVICEVALUE) ON_AFTERSETDEVICEVALUE(null);
	}	
}
...
function GetDevicesLogs_FN(params,ON_AFTERGETDEVICESLOGS, FOREACH_DEVICELOGS){
  ...
  var GetDeviceLogs = function(CALLBACK_FN,paramid){
    ...
    writelog("\tGetting device logs --> " + param.uuid + " [processing...]",false);
    ...
    GetDeviceLogs_FN(param.uuid,param.attribute,
                     function(device,datas){
                       ...
                       writelog("\tGetting device logs --> " + param.uuid + " [OK]");
                     },
                     function(err){
                       retval.push({
                         "uuid": param.uuid,
                       ...
                       writelog("\tGetting device logs --> " + param.uuid + " [ERROR]");
                     },
                     function(){					
  ...
  if (device){
    if (device.attributes){
      ...
      if (UrlReq){
        ...
        if (ONERROR_FN) ONERROR_FN("url replace error");	
        if(ON_AFTERGETDEVICELOGS) ON_AFTERGETDEVICELOGS();	
      }
    }
    else {
...
function GetDeviceByName_FN(devicename){
  writelog("GetDeviceByName : " + devicename,false);
    ...
    if (!(localdevice)) {
      for (var uuid in device.json) {
        if (device.json[uuid].name == devicename){
          writelog("GetDeviceByName : " + devicename + " -> " + "[FOUND]");	
  ...
  if(!(localdevice))
    writelog("GetDeviceByName : " + devicename + " -> " + "[NOT FOUND]");
...
function GetUnLoadedDeviceByName_FN(devicename){
  ...
  writelog("GetUnLoadedDeviceByName : " + devicename,false);
  ...
  if (localdevice){
    ...
    zipabox.LoadDevices(
      function(){	
        writelog(localdevice);			
        if (localdevice) {
          writelog("GetUnLoadedDeviceByName : " + devicename + " -> " + "[FOUND]");	
        }
        else {
          writelog("GetUnLoadedDeviceByName : " + devicename + " -> " + "[NOT FOUND]");
        }
...
function GetDeviceByUUID_FN(uuid){
  writelog("GetDeviceByUUID : " + (""+uuid),false);
  ...
  for(var iddevice in zipabox.devices){
    ...
    {
      writelog("GetDeviceByUUID : " + (""+uuid) + " -> " + "[FOUND]");
      return devicejson[uuid];	
    }				
  }
  
  writelog("GetDeviceByUUID : " + (""+uuid) + " -> " + "[NOT FOUND]");
  return null;
}
...
function ZipaboxToString() {
  ...
  //writelog("######### " + "ZIPABOX Informations" + " #########");
  retval += "######### " + "ZIPABOX Informations" + " #########" + "\r\n";
      ...
      for (var uuid in this.json) {
        ...
        if (this.name == "thermostats")
        {
          for(var endpoint in devicejson.endpoints)
          {
            ...
            retval += "\r\n\t\t" + endpoint + " : " + device_endpoint.uuid + "(" + device_endpoint.name + ")";
          }	
        }				
      }
      break;
      ...
      for (var uuid in this.json) {
        ...
        for (var attr in devicejson.attributes) {
          ...
          if (typeof (attribute.value) != "undefined") {
            ...
            retval += "\r\n\t" + (uuid + "(" + devicejson.name + ")" + " - " + attrname + "(" + attr + ")" + " = " + attrval);
          }
        }
      }				
  }	
...
function InitUser_FN(CALLBACK_FN){  
  ...
  writelog("Initialisation " + "[processing...]"); 
  ...
  GetDatas(
    ...
    /*ONSUCCESS_FN*/ function(err, res, body){ 
      ...
      if (zipabox.InitUser_Return.success)
      {
        ...
        zipabox.cookie = res.getAllHeaders()['Set-Cookie'];        
...
function GenerateTokenizedURI(){
  writelog("nonce: " + zipabox.nonce);

  var api_token = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_1, zipabox.nonce+zipabox.password, Utilities.Charset.UTF_8);  
  api_token = getSHA1(api_token);
  ...
  GetDatas(
    ...
    /*ONERROR_FN*/ function(err, res, body){
      ...
      if(err){
        ...
        msg = {statusCode: 5, msg:"Login error status code : " + res.getResponseCode()};        
        if (zipabox.events.OnLoginUserProgress) zipabox.events.OnLoginUserProgress(msg);
        writelog("Login error status code : " + res.getResponseCode());	
      }				
    }
  );	
}
...
function LoadDevices_FN(ON_AFTERLOADDEVICES,FOREACH_DEVICE_FN) {
  ...
  var LoadDevice = function(CALLBACK_FN,deviceid_int) {
    ...
    writelog("\tLoading device --> " + device + " [processing...]",false);
    ...
    GetDatas(
      ...
      /*ONSUCCESS_FN*/ function(err, res, body){
        ...
        {          
          ...
          writelog("\tLoading device --> " + device + " [OK]");
        ...
        {          
          writelog("\tLoading device --> " + device  + " [ERROR]");
          writelog("\tParse Query Result Error (device : " + device + ") : " + parseerr);
        }
      },
      /*ONERROR_FN*/ function(err, res, body){ 
